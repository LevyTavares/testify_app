...
import { useTemplates } from "../context/TemplateContext"; // <--- usar apenas o hook
...
export default function ReportsScreen() {
  ...
  const { templates } = useTemplates(); // <--- SUBSTITUA AQUI
  ...
  if (viewingResult && viewingTemplate) {
    // MUDANÇA: Garantimos que viewingTemplate não é nulo
  ...
  if (viewingTemplate) {
    ...
    const aggregateMetrics = {
      classAverage: (
        results.reduce((sum, r) => sum + parseFloat(r.score.split("/")[0]), 0) /
        (results.length || 1)
      ).toFixed(1),
      topPerformerName:
        results.length > 0
